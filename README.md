...
本项目仓库地址：https://github.com/YeYu-Wang/cursor-zh-hans-language-pack

后续会持续跟进 Cursor 新版本中的界面变化、硬编码文案和用户反馈，逐步补齐新增页面、菜单、Agent、Composer、Automations 等区域的简体中文汉化。

## 交流互助

使用中如果遇到安装、补丁、界面残留英文或版本兼容问题，可以扫码加入 QQ 群互助交流。

> 说明：Windows、macOS、Linux 均支持安装语言包和主程序补丁。不同 Cursor 版本里，设置页、菜单页等位置可能仍会残留部分英文。这通常来自平台差异、版本差异或主程序中的硬编码文案，不代表语言包安装失败。欢迎反馈具体版本号、系统信息和截图，便于继续补齐。
...
本扩展会在 Cursor 启动完成后自动检查主程序硬编码补丁。如果 Cursor 升级覆盖了 `resources/app` 内的文件，扩展会尽量自动补回，并提示重新加载窗口或完整重启 Cursor。

仍建议按顺序确认：

1. 重新安装最新的 `dist/cursor-zh-hans-language-pack.vsix`。
2. 打开命令面板，运行 `Configure Display Language`，确认显示语言为 `zh-cn`。
3. 保持设置 `cursorZhHans.autoApplyWorkbenchPatch` 为开启状态；如需手动处理，可运行 `Cursor 简体中文: 应用 Cursor Agent/Composer 主界面汉化补丁`。
4. 完整退出 Cursor，再重新打开。

如果编辑器、菜单、设置页全部变英文，优先检查第 1、2 步；如果只有 Agent、Composer、设置页局部变英文，通常会在下次启动自动补回，也可以手动重新运行第 3 步。
...
补丁目标是 Cursor 的 `resources/app/out/vs/workbench/workbench.desktop.main.js`，会在同目录生成备份：`workbench.desktop.main.js.cursor-zh-hans-workbench.bak`，并生成对应元数据文件记录原始文件和补丁后文件的摘要。升级后如果 Cursor 覆盖了主程序文件，扩展会刷新当前版本的备份，避免用旧版本备份覆盖新版本文件。应用或恢复后需要完整重启 Cursor。

自动定位覆盖常见安装位置：

- Windows：`%LOCALAPPDATA%\Programs\cursor\resources\app`
- macOS：`/Applications/Cursor.app/Contents/Resources/app`、`~/Applications/Cursor.app/Contents/Resources/app`
- Linux：`/usr/share/cursor/resources/app`、`/usr/lib/cursor/resources/app`、`/opt/Cursor/resources/app`、`/opt/cursor/resources/app`、`/app/share/cursor/resources/app`、AppImage/Snap 常见路径

如果自动定位失败，可以在设置里指定 `cursorZhHans.cursorAppRoot`，值应指向 Cursor 的 `resources/app` 目录。Linux 系统级安装目录可能需要当前用户具备写入权限；无权限时自动补丁会跳过，需要使用可写安装方式或手动以合适权限处理。
